...
export class HeatMapComponent {
  ...
  getBlockClasses(crypto: HeatmapCryptoData): string {
    const sizeClasses = this.getSizeClasses(crypto.size);
    const colorClasses = this.getColorClasses(crypto.color);
    
    return `${sizeClasses} ${colorClasses} rounded-lg p-4 cursor-pointer transition-all duration-200 hover:scale-105`;
  }
